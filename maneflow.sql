...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_add_child_account` (IN `p_parent_user_id` INT, IN `p_child_first_name` VARCHAR(100), IN `p_child_last_name` VARCHAR(100), IN `p_child_dob` DATE, IN `p_child_gender` ENUM('female','male','other','prefer_not_to_say'))   BEGIN
  DECLARE v_child_age INT;
  DECLARE v_child_email VARCHAR(255);
  DECLARE v_random_password VARCHAR(255);
  DECLARE v_child_user_id INT;
  
  -- Calculate child's age
  SET v_child_age = TIMESTAMPDIFF(YEAR, p_child_dob, CURDATE());
  
  -- Validate parent is at least 13 years old
  IF (SELECT TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE()) FROM users WHERE user_id = p_parent_user_id) < 13 THEN
    SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'Parent must be at least 13 years old to create child accounts';
  END IF;
  
  -- Validate child is under 13
  IF v_child_age >= 13 THEN
    SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'Child must be under 13 years old. Users 13+ should create their own accounts.';
  END IF;
  
  -- Generate unique child email
  SET v_child_email = CONCAT(
    (SELECT email FROM users WHERE user_id = p_parent_user_id),
    '.', 
    LOWER(REPLACE(p_child_first_name, ' ', '')), 
    '_', 
    UNIX_TIMESTAMP(),
    '@maneflow.child'
  );
  
  -- Generate random password (in practice, use a secure random generator)
  SET v_random_password = MD5(CONCAT(RAND(), UNIX_TIMESTAMP(), v_child_email));
  ...
  SET v_child_user_id = LAST_INSERT_ID();
  
  -- Log the activity
  INSERT INTO parent_child_activity_log (parent_user_id, child_user_id, action_type, action_details)
  VALUES (p_parent_user_id, v_child_user_id, 'created', CONCAT('Created child account for ', p_child_first_name));
  
  -- Return the new child user ID
  SELECT v_child_user_id AS child_user_id, v_child_email AS child_email;
  
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_generate_age_recommendations` (IN `p_profile_id` INT)   BEGIN
  DECLARE v_user_id INT;
  DECLARE v_age_group VARCHAR(20);
  DECLARE v_hair_type_id INT;
  ...
  SELECT 
    ...
    u.date_of_birth,
    uhp.hair_type_id
  INTO v_user_id, v_user_dob, v_hair_type_id
  FROM user_hair_profiles uhp
  JOIN users u ON uhp.user_id = u.user_id
  WHERE uhp.profile_id = p_profile_id;
  
  -- Calculate age group
  SET v_age_group = CASE 
    WHEN TIMESTAMPDIFF(YEAR, v_user_dob, CURDATE()) < 13 THEN 'child'
    WHEN TIMESTAMPDIFF(YEAR, v_user_dob, CURDATE()) BETWEEN 13 AND 19 THEN 'teen'
    WHEN TIMESTAMPDIFF(YEAR, v_user_dob, CURDATE()) BETWEEN 20 AND 35 THEN 'young_adult'
    WHEN TIMESTAMPDIFF(YEAR, v_user_dob, CURDATE()) BETWEEN 36 AND 50 THEN 'adult'
    WHEN TIMESTAMPDIFF(YEAR, v_user_dob, CURDATE()) BETWEEN 51 AND 65 THEN 'middle_aged'
    ELSE 'senior'
  END;
  ...
  -- Insert age-appropriate product recommendations
  INSERT INTO user_recommendations 
  (profile_id, recommendation_type, product_id, priority, personalized_note, age_consideration)
  SELECT 
    p_profile_id,
    'product',
    phtc.product_id,
    CASE 
      WHEN paa.suitability_score >= 9 THEN 'high'
      WHEN paa.suitability_score >= 7 THEN 'medium'
      ELSE 'low'
    END,
    CONCAT('Recommended for your hair type. ', phtc.notes),
    CONCAT('Age-appropriate (Score: ', paa.suitability_score, '/10). ', 
           COALESCE(paa.age_specific_notes, ''),
           CASE WHEN paa.warnings IS NOT NULL 
           THEN CONCAT(' WARNING: ', paa.warnings) ELSE '' END)
  FROM product_hair_type_compatibility phtc
  JOIN product_age_appropriateness paa 
    ON phtc.product_id = paa.product_id
  WHERE phtc.hair_type_id = v_hair_type_id
    AND paa.age_group = v_age_group
    AND phtc.compatibility_score >= 7
    AND paa.suitability_score >= 6
  ORDER BY 
    paa.suitability_score DESC,
    phtc.compatibility_score DESC
  LIMIT 10;
  
  -- Insert age-appropriate method recommendations
  INSERT INTO user_recommendations 
  (profile_id, recommendation_type, method_id, priority, personalized_note, age_consideration)
  SELECT 
    p_profile_id,
    'method',
    mhtc.method_id,
    CASE 
      WHEN mas.suitability_score >= 9 THEN 'high'
      WHEN mas.suitability_score >= 7 THEN 'medium'
      ELSE 'low'
    END,
    CONCAT('Effective method for your hair type. ', mhtc.notes),
    CONCAT('Age-appropriate (Score: ', mas.suitability_score, '/10). ',
           COALESCE(mas.age_modifications, ''),
           CASE WHEN mas.precautions IS NOT NULL 
           THEN CONCAT(' PRECAUTION: ', mas.precautions) ELSE '' END)
  FROM method_hair_type_compatibility mhtc
  JOIN method_age_suitability mas 
    ON mhtc.method_id = mas.method_id
  WHERE mhtc.hair_type_id = v_hair_type_id
    AND mas.age_group = v_age_group
    AND mhtc.effectiveness_score >= 7
    AND mas.suitability_score >= 6
  ORDER BY 
    mas.suitability_score DESC,
    mhtc.effectiveness_score DESC
  LIMIT 7;
  
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_get_parent_children` (IN `p_parent_user_id` INT)   BEGIN
  SELECT 
    c.user_id,
    c.first_name,
    c.last_name,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_verify_parent_child` (IN `p_parent_user_id` INT, IN `p_child_user_id` INT)   BEGIN
  ...
  WHERE user_id = p_child_user_id
    ...
    AND is_child_account = 1;
  
  IF v_is_valid = 0 THEN
    SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'Invalid parent-child relationship or child account not found';
  END IF;
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_get_age_group` (`p_date_of_birth` DATE) RETURNS VARCHAR(20) CHARSET utf8mb4 COLLATE utf8mb4_general_ci DETERMINISTIC BEGIN
  DECLARE v_age INT;
  DECLARE v_age_group VARCHAR(20);
  ...
  SET v_age_group = CASE 
    WHEN v_age < 13 THEN 'child'
    WHEN v_age BETWEEN 13 AND 19 THEN 'teen'
    WHEN v_age BETWEEN 20 AND 35 THEN 'young_adult'
    WHEN v_age BETWEEN 36 AND 50 THEN 'adult'
    WHEN v_age BETWEEN 51 AND 65 THEN 'middle_aged'
    ELSE 'senior'
  END;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_get_parent_id` (`p_child_user_id` INT) RETURNS INT(11) DETERMINISTIC BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_is_parent` (`p_user_id` INT) RETURNS TINYINT(1) DETERMINISTIC BEGIN
...
CREATE TABLE `age_hair_factors` (
  ...
  `age_group` enum('child','teen','young_adult','adult','middle_aged','senior') NOT NULL,
...
CREATE TABLE `age_specific_concerns` (
  `age_concern_id` int(11) NOT NULL,
  `age_group` enum('child','teen','young_adult','adult','middle_aged','senior') NOT NULL,
  `concern_name` varchar(200) NOT NULL,
  `description` text DEFAULT NULL,
  `prevalence` enum('rare','occasional','common','very_common') DEFAULT 'common',
  `typical_causes` text DEFAULT NULL,
...
CREATE TABLE `content_age_appropriateness` (
  `content_age_id` int(11) NOT NULL,
  `content_id` int(11) NOT NULL,
  `age_group` enum('child','teen','young_adult','adult','middle_aged','senior') NOT NULL,
  `is_appropriate` tinyint(1) DEFAULT 1,
  `age_specific_notes` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `content_hair_type_relevance` (
  ...
  `relevance_score` int(11) DEFAULT NULL CHECK (`relevance_score` between 1 and 10)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `diagnosis_causes` (
  ...
  `category` enum('product','practice','nutrition','medical','environmental','hormonal','genetic','stress','other') DEFAULT NULL,
  `description` text DEFAULT NULL,
...
CREATE TABLE `educational_content` (
  ...
  `content_type` enum('article','video','infographic','tutorial','tip','guide','quiz') DEFAULT NULL,
...
CREATE TABLE `growth_forecasts` (
  ...
  `confidence_level` decimal(3,2) DEFAULT NULL CHECK (`confidence_level` between 0 and 1),
  `based_on_data_points` int(11) DEFAULT 0,
...
(7, 3, '2026-01-12', 7.00, 0.17, 2, -6.00, 'Based on 2 data points. Average growth rate: -6.00 inches/month.', '2025-12-12 21:31:42'),
(8, 3, '2026-02-12', 1.00, 0.17, 2, -6.00, 'Based on 2 data points. Average growth rate: -6.00 inches/month.', '2025-12-12 21:31:42'),
(9, 3, '2026-03-12', 0.00, 0.17, 2, -6.00, 'Based on 2 data points. Average growth rate: -6.00 inches/month.', '2025-12-12 21:31:42'),
(10, 3, '2026-04-12', 0.00, 0.17, 2, -6.00, 'Based on 2 data points. Average growth rate: -6.00 inches/month.', '2025-12-12 21:31:42'),
(11, 3, '2026-05-12', 0.00, 0.17, 2, -6.00, 'Based on 2 data points. Average growth rate: -6.00 inches/month.', '2025-12-12 21:31:42'),
(12, 3, '2026-06-12', 0.00, 0.17, 2, -6.00, 'Based on 2 data points. Average growth rate: -6.00 inches/month.', '2025-12-12 21:31:42');
...
CREATE TABLE `growth_methods` (
  ...
  `category` enum('protective_styling','scalp_care','nutrition','trimming','massage','treatment','lifestyle','hot_oil','deep_conditioning','inversion','supplements','other') DEFAULT NULL,
  `description` text DEFAULT NULL,
  ...
  `scientific_backing` text DEFAULT NULL,
...
CREATE TABLE `growth_milestones` (
  ...
  `celebration_message` text DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `hair_care_routines` (
  ...
  `profile_id` int(11) NOT NULL,
  `routine_name` varchar(200) DEFAULT NULL,
  `routine_type` enum('morning','night','wash_day','weekly','bi-weekly','monthly','custom') DEFAULT NULL,
  `description` text DEFAULT NULL,
  `is_active` tinyint(1) DEFAULT 1,
  `last_update_alert` date DEFAULT NULL,
  `update_frequency_days` int(11) DEFAULT 90,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
...
(31, 6, 'Morning Routine', 'morning', 'Personalized morning hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-12', 90, '2025-12-12 23:49:08', '2025-12-13 16:32:47'),
(32, 6, 'Night Routine', 'night', 'Personalized night hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-12', 90, '2025-12-12 23:49:08', '2025-12-13 16:32:47'),
(33, 6, 'Wash_day Routine', 'wash_day', 'Personalized wash_day hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-12', 90, '2025-12-12 23:49:08', '2025-12-13 16:32:47'),
...
(37, 6, 'Morning Routine', 'morning', 'Personalized morning hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-13', 90, '2025-12-13 16:32:47', '2025-12-13 16:32:49'),
(38, 6, 'Night Routine', 'night', 'Personalized night hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-13', 90, '2025-12-13 16:32:47', '2025-12-13 16:32:49'),
(39, 6, 'Wash_day Routine', 'wash_day', 'Personalized wash_day hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-13', 90, '2025-12-13 16:32:47', '2025-12-13 16:32:49'),
(40, 6, 'Morning Routine', 'morning', 'Personalized morning hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-13', 90, '2025-12-13 16:32:49', '2025-12-13 16:32:49'),
(41, 6, 'Night Routine', 'night', 'Personalized night hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-13', 90, '2025-12-13 16:32:49', '2025-12-13 16:32:49'),
(42, 6, 'Wash_day Routine', 'wash_day', 'Personalized wash_day hair care routine for Type 4A - Coily Soft hair type', 0, '2025-12-13', 90, '2025-12-13 16:32:49', '2025-12-13 16:32:49'),
(43, 6, 'Morning Routine', 'morning', 'Personalized morning hair care routine for Type 4A - Coily Soft hair type', 1, '2025-12-13', 90, '2025-12-13 16:32:49', '2025-12-13 16:32:49'),
(44, 6, 'Night Routine', 'night', 'Personalized night hair care routine for Type 4A - Coily Soft hair type', 1, '2025-12-13', 90, '2025-12-13 16:32:49', '2025-12-13 16:32:49'),
(45, 6, 'Wash_day Routine', 'wash_day', 'Personalized wash_day hair care routine for Type 4A - Coily Soft hair type', 1, '2025-12-13', 90, '2025-12-13 16:32:49', '2025-12-13 16:32:49');
...
CREATE TABLE `hair_concerns` (
  ...
  `description` text DEFAULT NULL,
  `severity_level` enum('mild','moderate','severe') DEFAULT NULL,
...
CREATE TABLE `hair_growth_progress` (
  ...
  `hair_health_rating` int(11) DEFAULT NULL CHECK (`hair_health_rating` between 1 and 10),
  `photo_url` varchar(500) DEFAULT NULL,
  `notes` text DEFAULT NULL,
  `measurement_type` enum('manual','photo_analysis','routine') DEFAULT 'manual',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `hair_pitfalls` (
  `pitfall_id` int(11) NOT NULL,
  `pitfall_name` varchar(200) NOT NULL,
  `category` enum('product','practice','tool','ingredient','styling','environmental','chemical','heat','manipulation','other') DEFAULT NULL,
  `description` text DEFAULT NULL,
  `why_harmful` text DEFAULT NULL,
  `severity` enum('low','medium','high','critical') DEFAULT NULL,
  `alternative_suggestion` text DEFAULT NULL,
  `long_term_effects` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `hair_symptoms` (
  ...
  `category` enum('breakage','dryness','dandruff','shedding','scalp_irritation','thinning','split_ends','lack_of_growth','texture_change','other') NOT NULL,
  `description` text DEFAULT NULL,
  `severity_indicators` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `hair_types` (
  ...
  `category` enum('straight','wavy','curly','coily') NOT NULL,
  `description` text DEFAULT NULL,
...
CREATE TABLE `method_age_suitability` (
  ...
  `method_id` int(11) NOT NULL,
  `age_group` enum('child','teen','young_adult','adult','middle_aged','senior') NOT NULL,
  `suitability_score` int(11) DEFAULT NULL CHECK (`suitability_score` between 1 and 10),
  `age_modifications` text DEFAULT NULL,
...
CREATE TABLE `method_hair_type_compatibility` (
  ...
  `method_id` int(11) NOT NULL,
  `hair_type_id` int(11) NOT NULL,
  `effectiveness_score` int(11) DEFAULT NULL CHECK (`effectiveness_score` between 1 and 10),
  `notes` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `parent_child_activity_log` (
  ...
  `action_type` enum('created','viewed_profile','edited_profile','viewed_progress','viewed_recommendations','deleted') NOT NULL,
...
(29, 4, 6, 'viewed_profile', 'Parent viewed child profile', '2025-12-13 16:35:32');
...
CREATE TABLE `pitfall_hair_type_applicability` (
  `applicability_id` int(11) NOT NULL,
  `pitfall_id` int(11) NOT NULL,
  `hair_type_id` int(11) NOT NULL,
  `risk_level` enum('low','medium','high','critical') DEFAULT NULL,
  `specific_notes` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `products` (
  ...
  `category` enum('shampoo','conditioner','deep_conditioner','leave_in','oil','serum','mask','styling','supplement','tool','protein_treatment','moisturizer','detangler','scalp_treatment','other') DEFAULT NULL,
  `description` text DEFAULT NULL,
...
CREATE TABLE `product_age_appropriateness` (
  `appropriateness_id` int(11) NOT NULL,
  `product_id` int(11) NOT NULL,
  `age_group` enum('child','teen','young_adult','adult','middle_aged','senior') NOT NULL,
  `suitability_score` int(11) DEFAULT NULL CHECK (`suitability_score` between 1 and 10),
  `age_specific_notes` text DEFAULT NULL,
...
CREATE TABLE `product_hair_type_compatibility` (
  ...
  `product_id` int(11) NOT NULL,
  `hair_type_id` int(11) NOT NULL,
  `compatibility_score` int(11) DEFAULT NULL CHECK (`compatibility_score` between 1 and 10),
  `notes` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `protective_styles` (
  ...
  `category` enum('braids','twists','locs','wigs','weaves','updos','buns','cornrows','other') DEFAULT NULL,
  `description` text DEFAULT NULL,
  `difficulty_level` enum('beginner','intermediate','advanced','expert') DEFAULT NULL,
  `expected_longevity_days` int(11) DEFAULT NULL,
  `maintenance_instructions` text DEFAULT NULL,
  `installation_time` varchar(50) DEFAULT NULL,
  `cost_range` varchar(50) DEFAULT NULL,
  `common_mistakes` text DEFAULT NULL,
  `removal_instructions` text DEFAULT NULL,
  `image_url` varchar(500) DEFAULT NULL,
  `video_tutorial_url` varchar(500) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `routine_completion_log` (
  ...
  `skipped_steps` text DEFAULT NULL,
  `notes` text DEFAULT NULL,
...
(1, 43, 6, '2025-12-14', NULL, NULL, 'Completed via app', NULL, '2025-12-14 14:13:00');
...
CREATE TABLE `routine_steps` (
  ...
  `frequency_note` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `symptom_cause_mapping` (
  ...
  `symptom_id` int(11) NOT NULL,
  `cause_id` int(11) NOT NULL,
  `likelihood_score` int(11) DEFAULT NULL CHECK (`likelihood_score` between 1 and 10),
...
CREATE TABLE `treatment_solutions` (
  ...
  `solution_type` enum('product','method','lifestyle_change','professional_treatment','dietary','medical') DEFAULT NULL,
  `description` text DEFAULT NULL,
  `instructions` text DEFAULT NULL,
  `expected_timeframe` varchar(100) DEFAULT NULL,
  `product_id` int(11) DEFAULT NULL,
  `method_id` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `users` (
  ...
  `gender` enum('female','male','other','prefer_not_to_say') DEFAULT 'female',
  ...
  `hormonal_status` enum('pre_puberty','puberty','reproductive','perimenopause','menopause','post_menopause','andropause','not_applicable') DEFAULT 'not_applicable',
  `parent_user_id` int(11) DEFAULT NULL,
  `is_child_account` tinyint(1) DEFAULT 0,
  `child_age_when_created` int(11) DEFAULT NULL,
...
(2, 'happy@gmail.com', '$2y$10$vGr5w47.UsgvvEYVyohsNuqdMan6YpSgqYNILyVGSlUpIkXkyjrem', 'Wylie', 'Mccray', 'other', NULL, '2025-12-12 21:28:43', '2025-12-12 21:28:50', '2025-12-12 21:28:50', 1, 1, NULL, 'not_applicable', NULL, 0, NULL, 0, NULL),
(3, 'joycelyn@gmail.com', '$2y$10$wnH/T/MsuUgORlHkZiXh7uDn7a0wx/XP0JLXbGYx.pB1Vc94SoHS2', 'Joycelyn', 'Allan', 'female', '2002-06-20', '2025-12-12 22:01:39', '2025-12-12 22:01:49', '2025-12-12 22:01:49', 1, 1, NULL, 'not_applicable', NULL, 0, NULL, 0, NULL),
(4, 'bell@gmail.com', '$2y$10$leunaUAreklFFlj41EHg7e4X7dG34I.z9s5KTaR1609Q9yiHTcMQO', 'Hanna', 'Bell', 'prefer_not_to_say', '1999-11-16', '2025-12-12 22:46:49', '2025-12-13 17:09:42', '2025-12-13 17:09:42', 1, 1, NULL, 'not_applicable', NULL, 0, NULL, 0, NULL),
...
--
DELIMITER $$
CREATE TRIGGER `trg_prevent_child_as_parent` BEFORE INSERT ON `users` FOR EACH ROW BEGIN
  IF NEW.parent_user_id IS NOT NULL THEN
    -- Check if the parent is also a child account
    IF EXISTS (SELECT 1 FROM users WHERE user_id = NEW.parent_user_id AND is_child_account = 1) THEN
      SIGNAL SQLSTATE '45000' 
      SET MESSAGE_TEXT = 'Child accounts cannot create child accounts';
    END IF;
  END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_update_age_recommendations` AFTER UPDATE ON `users` FOR EACH ROW BEGIN
  DECLARE v_old_age_group VARCHAR(20);
  DECLARE v_new_age_group VARCHAR(20);
  
  -- Calculate old age group
  SET v_old_age_group = CASE 
    WHEN TIMESTAMPDIFF(YEAR, OLD.date_of_birth, CURDATE()) < 13 THEN 'child'
    WHEN TIMESTAMPDIFF(YEAR, OLD.date_of_birth, CURDATE()) BETWEEN 13 AND 19 THEN 'teen'
    WHEN TIMESTAMPDIFF(YEAR, OLD.date_of_birth, CURDATE()) BETWEEN 20 AND 35 THEN 'young_adult'
    WHEN TIMESTAMPDIFF(YEAR, OLD.date_of_birth, CURDATE()) BETWEEN 36 AND 50 THEN 'adult'
    WHEN TIMESTAMPDIFF(YEAR, OLD.date_of_birth, CURDATE()) BETWEEN 51 AND 65 THEN 'middle_aged'
    ELSE 'senior'
  END;
  
  -- Calculate new age group
  SET v_new_age_group = CASE 
    WHEN TIMESTAMPDIFF(YEAR, NEW.date_of_birth, CURDATE()) < 13 THEN 'child'
    WHEN TIMESTAMPDIFF(YEAR, NEW.date_of_birth, CURDATE()) BETWEEN 13 AND 19 THEN 'teen'
    WHEN TIMESTAMPDIFF(YEAR, NEW.date_of_birth, CURDATE()) BETWEEN 20 AND 35 THEN 'young_adult'
    WHEN TIMESTAMPDIFF(YEAR, NEW.date_of_birth, CURDATE()) BETWEEN 36 AND 50 THEN 'adult'
    WHEN TIMESTAMPDIFF(YEAR, NEW.date_of_birth, CURDATE()) BETWEEN 51 AND 65 THEN 'middle_aged'
    ELSE 'senior'
  END;
  
  IF v_old_age_group != v_new_age_group THEN
    -- Flag user's recommendations for regeneration
    UPDATE user_recommendations ur
    JOIN user_hair_profiles uhp ON ur.profile_id = uhp.profile_id
    SET ur.is_active = 0
    WHERE uhp.user_id = NEW.user_id;
  END IF;
END
$$
DELIMITER ;
...
CREATE TABLE `user_diagnoses` (
  ...
  `symptoms_reported` text DEFAULT NULL,
  `identified_causes` text DEFAULT NULL,
  `recommended_solutions` text DEFAULT NULL,
...
CREATE TABLE `user_hair_concerns` (
  `user_concern_id` int(11) NOT NULL,
  `profile_id` int(11) NOT NULL,
  `concern_id` int(11) NOT NULL,
  `severity` enum('mild','moderate','severe') DEFAULT NULL,
  `noted_date` date DEFAULT curdate(),
...
CREATE TABLE `user_hair_profiles` (
  `profile_id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL,
  `hair_type_id` int(11) DEFAULT NULL,
  `hair_density` enum('low','medium','high') DEFAULT NULL,
  `hair_porosity` enum('low','medium','high') DEFAULT NULL,
  `scalp_type` enum('dry','normal','oily','combination','sensitive') DEFAULT NULL,
  `current_length` decimal(5,2) DEFAULT NULL,
  `goal_length` decimal(5,2) DEFAULT NULL,
  `hair_texture` varchar(50) DEFAULT NULL,
  `elasticity` enum('low','normal','high') DEFAULT NULL,
  `is_color_treated` tinyint(1) DEFAULT 0,
  `is_chemically_treated` tinyint(1) DEFAULT 0,
  `chemical_treatment_type` varchar(100) DEFAULT NULL,
  `growth_goal` text DEFAULT NULL,
...
--
-- Triggers `user_hair_profiles`
--
DELIMITER $$
CREATE TRIGGER `trg_log_profile_view` AFTER UPDATE ON `user_hair_profiles` FOR EACH ROW BEGIN
  DECLARE v_parent_id INT;
  
  -- Check if this is a child account
  SELECT parent_user_id INTO v_parent_id
  FROM users
  WHERE user_id = NEW.user_id AND is_child_account = 1;
  
  -- Log if parent is viewing/editing child's profile
  IF v_parent_id IS NOT NULL THEN
    INSERT INTO parent_child_activity_log (parent_user_id, child_user_id, action_type, action_details)
    VALUES (v_parent_id, NEW.user_id, 'edited_profile', 'Parent updated child profile');
  END IF;
END
$$
DELIMITER ;
...
CREATE TABLE `user_notifications` (
  ...
  `user_id` int(11) NOT NULL,
  `notification_type` enum('routine_update','milestone_achieved','growth_forecast','diagnosis_follow_up','style_reminder','product_recommendation','general') DEFAULT NULL,
...
INSERT INTO `user_notifications` (`notification_id`, `user_id`, `notification_type`, `title`, `message`, `is_read`, `action_url`, `created_at`) VALUES
(1, 3, 'general', 'Welcome to ManeFlow', 'Thanks for joining our community! Complete your profile to get started.', 0, 'profile.php', '2025-12-13 20:16:47'),
(2, 3, 'routine_update', 'Time for your routine', 'It\'s wash day! Check out your personalized routine.', 0, 'routine_detail.php?id=12', '2025-12-13 20:16:47'),
(3, 3, 'product_recommendation', 'New Product Match', 'We found a new conditioner that matches your hair profile.', 1, 'recommendations.php', '2025-12-13 20:16:47');
...
CREATE TABLE `user_recommendations` (
  ...
  `profile_id` int(11) NOT NULL,
  `recommendation_type` enum('product','method','pitfall_avoidance','general_tip','routine','style','treatment') DEFAULT NULL,
  `product_id` int(11) DEFAULT NULL,
  `method_id` int(11) DEFAULT NULL,
  `pitfall_id` int(11) DEFAULT NULL,
  `style_id` int(11) DEFAULT NULL,
  `priority` enum('low','medium','high','critical') DEFAULT NULL,
  `personalized_note` text DEFAULT NULL,
  `reason_for_recommendation` text DEFAULT NULL,
  `generated_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `is_active` tinyint(1) DEFAULT 1,
  `user_dismissed` tinyint(1) DEFAULT 0,
  `age_consideration` text DEFAULT NULL,
...
--
-- Stand-in structure for view `v_age_appropriate_recommendations`
-- (See below for the actual view)
--
CREATE TABLE `v_age_appropriate_recommendations` (
`recommendation_id` int(11)
,`profile_id` int(11)
,`recommendation_type` enum('product','method','pitfall_avoidance','general_tip','routine','style','treatment')
,`product_id` int(11)
,`method_id` int(11)
,`pitfall_id` int(11)
,`style_id` int(11)
...
,`is_active` tinyint(1)
,`user_dismissed` tinyint(1)
,`age_consideration` text
,`age_modified` tinyint(1)
,`age_group` varchar(11)
,`hormonal_status` enum('pre_puberty','puberty','reproductive','perimenopause','menopause','post_menopause','andropause','not_applicable')
,`current_age` bigint(21)
,`product_age_score` int(11)
,`product_age_notes` text
,`product_age_warnings` text
,`method_age_score` int(11)
,`method_age_modifications` text
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `v_parent_children`
-- (See below for the actual view)
--
...
,`child_user_id` int(11)
,`child_email` varchar(255)
,`child_first_name` varchar(100)
,`child_last_name` varchar(100)
,`child_dob` date
,`child_current_age` bigint(21)
,`child_gender` enum('female','male','other','prefer_not_to_say')
,`child_account_created` timestamp
,`child_is_active` tinyint(1)
,`has_profile` int(1)
);
...
--
-- Stand-in structure for view `v_users_with_age`
-- (See below for the actual view)
--
CREATE TABLE `v_users_with_age` (
`user_id` int(11)
,`email` varchar(255)
...
,`is_active` tinyint(1)
,`offline_mode_enabled` tinyint(1)
,`last_sync` timestamp
,`hormonal_status` enum('pre_puberty','puberty','reproductive','perimenopause','menopause','post_menopause','andropause','not_applicable')
,`current_age` bigint(21)
,`age_group` varchar(11)
...
--
-- Structure for view `v_age_appropriate_recommendations`
--
DROP TABLE IF EXISTS `v_age_appropriate_recommendations`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_age_appropriate_recommendations`  AS SELECT `ur`.`recommendation_id` AS `recommendation_id`, `ur`.`profile_id` AS `profile_id`, `ur`.`recommendation_type` AS `recommendation_type`, `ur`.`product_id` AS `product_id`, `ur`.`method_id` AS `method_id`, `ur`.`pitfall_id` AS `pitfall_id`, `ur`.`style_id` AS `style_id`, `ur`.`priority` AS `priority`, `ur`.`personalized_note` AS `personalized_note`, `ur`.`reason_for_recommendation` AS `reason_for_recommendation`, `ur`.`generated_at` AS `generated_at`, `ur`.`is_active` AS `is_active`, `ur`.`user_dismissed` AS `user_dismissed`, `ur`.`age_consideration` AS `age_consideration`, `ur`.`age_modified` AS `age_modified`, CASE WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) < 13 THEN 'child' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 13 and 19 THEN 'teen' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 20 and 35 THEN 'young_adult' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 36 and 50 THEN 'adult' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 51 and 65 THEN 'middle_aged' ELSE 'senior' END AS `age_group`, `u`.`hormonal_status` AS `hormonal_status`, timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) AS `current_age`, `paa`.`suitability_score` AS `product_age_score`, `paa`.`age_specific_notes` AS `product_age_notes`, `paa`.`warnings` AS `product_age_warnings`, `mas`.`suitability_score` AS `method_age_score`, `mas`.`age_modifications` AS `method_age_modifications` FROM ((((`user_recommendations` `ur` join `user_hair_profiles` `uhp` on(`ur`.`profile_id` = `uhp`.`profile_id`)) join `users` `u` on(`uhp`.`user_id` = `u`.`user_id`)) left join `product_age_appropriateness` `paa` on(`ur`.`product_id` = `paa`.`product_id` and `paa`.`age_group` = case when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) < 13 then 'child' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 13 and 19 then 'teen' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 20 and 35 then 'young_adult' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 36 and 50 then 'adult' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 51 and 65 then 'middle_aged' else 'senior' end)) left join `method_age_suitability` `mas` on(`ur`.`method_id` = `mas`.`method_id` and `mas`.`age_group` = case when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) < 13 then 'child' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 13 and 19 then 'teen' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 20 and 35 then 'young_adult' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 36 and 50 then 'adult' when timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 51 and 65 then 'middle_aged' else 'senior' end)) ;

-- --------------------------------------------------------

--
-- Structure for view `v_parent_children`
--
DROP TABLE IF EXISTS `v_parent_children`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_parent_children`  AS SELECT `p`.`user_id` AS `parent_user_id`, `p`.`email` AS `parent_email`, `p`.`first_name` AS `parent_first_name`, `p`.`last_name` AS `parent_last_name`, `c`.`user_id` AS `child_user_id`, `c`.`email` AS `child_email`, `c`.`first_name` AS `child_first_name`, `c`.`last_name` AS `child_last_name`, `c`.`date_of_birth` AS `child_dob`, timestampdiff(YEAR,`c`.`date_of_birth`,curdate()) AS `child_current_age`, `c`.`gender` AS `child_gender`, `c`.`created_at` AS `child_account_created`, `c`.`is_active` AS `child_is_active`, CASE WHEN exists(select 1 from `user_hair_profiles` where `user_hair_profiles`.`user_id` = `c`.`user_id` limit 1) THEN 1 ELSE 0 END AS `has_profile` FROM (`users` `p` join `users` `c` on(`p`.`user_id` = `c`.`parent_user_id`)) WHERE `c`.`is_child_account` = 1 ;

-- --------------------------------------------------------

--
-- Structure for view `v_users_with_age`
--
DROP TABLE IF EXISTS `v_users_with_age`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_users_with_age`  AS SELECT `u`.`user_id` AS `user_id`, `u`.`email` AS `email`, `u`.`password_hash` AS `password_hash`, `u`.`first_name` AS `first_name`, `u`.`last_name` AS `last_name`, `u`.`gender` AS `gender`, `u`.`date_of_birth` AS `date_of_birth`, `u`.`created_at` AS `created_at`, `u`.`updated_at` AS `updated_at`, `u`.`last_login` AS `last_login`, `u`.`is_active` AS `is_active`, `u`.`offline_mode_enabled` AS `offline_mode_enabled`, `u`.`last_sync` AS `last_sync`, `u`.`hormonal_status` AS `hormonal_status`, timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) AS `current_age`, CASE WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) < 13 THEN 'child' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 13 and 19 THEN 'teen' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 20 and 35 THEN 'young_adult' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 36 and 50 THEN 'adult' WHEN timestampdiff(YEAR,`u`.`date_of_birth`,curdate()) between 51 and 65 THEN 'middle_aged' ELSE 'senior' END AS `age_group` FROM `users` AS `u` ;
...
--
-- Indexes for table `growth_methods`
--
ALTER TABLE `growth_methods`
  ADD PRIMARY KEY (`method_id`),
  ADD KEY `idx_method_category` (`category`);

--
-- Indexes for table `growth_milestones`
--
ALTER TABLE `growth_milestones`
  ADD PRIMARY KEY (`milestone_id`),
  ADD KEY `idx_milestone_profile` (`profile_id`);

--
-- Indexes for table `hair_care_routines`
--
ALTER TABLE `hair_care_routines`
  ADD PRIMARY KEY (`routine_id`),
  ADD KEY `idx_routine_profile` (`profile_id`),
  ADD KEY `idx_routine_type` (`routine_type`);

--
-- Indexes for table `hair_concerns`
--
ALTER TABLE `hair_concerns`
  ADD PRIMARY KEY (`concern_id`),
  ADD UNIQUE KEY `concern_name` (`concern_name`);

--
-- Indexes for table `hair_growth_progress`
--
ALTER TABLE `hair_growth_progress`
  ADD PRIMARY KEY (`progress_id`),
  ADD KEY `idx_progress_profile_date` (`profile_id`,`measurement_date`);

--
-- Indexes for table `hair_pitfalls`
--
ALTER TABLE `hair_pitfalls`
  ADD PRIMARY KEY (`pitfall_id`),
  ADD KEY `idx_pitfall_category` (`category`),
  ADD KEY `idx_pitfall_severity` (`severity`);

--
-- Indexes for table `hair_symptoms`
--
ALTER TABLE `hair_symptoms`
...
--
-- Indexes for table `method_age_suitability`
--
ALTER TABLE `method_age_suitability`
  ADD PRIMARY KEY (`suitability_id`),
  ADD KEY `idx_method_age` (`method_id`,`age_group`);

--
-- Indexes for table `method_hair_type_compatibility`
--
ALTER TABLE `method_hair_type_compatibility`
  ADD PRIMARY KEY (`compatibility_id`),
  ADD KEY `method_id` (`method_id`),
  ADD KEY `hair_type_id` (`hair_type_id`);

--
-- Indexes for table `parent_child_activity_log`
--
ALTER TABLE `parent_child_activity_log`
  ADD PRIMARY KEY (`log_id`),
  ADD KEY `idx_parent_activity` (`parent_user_id`),
  ADD KEY `idx_child_activity` (`child_user_id`);

--
-- Indexes for table `pitfall_hair_type_applicability`
--
ALTER TABLE `pitfall_hair_type_applicability`
  ADD PRIMARY KEY (`applicability_id`),
...
--
ALTER TABLE `age_hair_factors`
  MODIFY `factor_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=21;

--
-- AUTO_INCREMENT for table `age_specific_concerns`
--
ALTER TABLE `age_specific_concerns`
  MODIFY `age_concern_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=15;

--
-- AUTO_INCREMENT for table `content_age_appropriateness`
--
ALTER TABLE `content_age_appropriateness`
  MODIFY `content_age_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=55;

--
-- AUTO_INCREMENT for table `content_hair_type_relevance`
--
ALTER TABLE `content_hair_type_relevance`
  MODIFY `relevance_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=25;

--
-- AUTO_INCREMENT for table `diagnosis_causes`
--
ALTER TABLE `diagnosis_causes`
  MODIFY `cause_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=34;
...
--
ALTER TABLE `growth_forecasts`
  MODIFY `forecast_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;

--
-- AUTO_INCREMENT for table `growth_methods`
--
ALTER TABLE `growth_methods`
  MODIFY `method_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT for table `growth_milestones`
--
ALTER TABLE `growth_milestones`
  MODIFY `milestone_id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `hair_care_routines`
--
ALTER TABLE `hair_care_routines`
  MODIFY `routine_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=46;

--
-- AUTO_INCREMENT for table `hair_concerns`
--
ALTER TABLE `hair_concerns`
  MODIFY `concern_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;

--
-- AUTO_INCREMENT for table `hair_growth_progress`
--
ALTER TABLE `hair_growth_progress`
  MODIFY `progress_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT for table `hair_pitfalls`
--
ALTER TABLE `hair_pitfalls`
  MODIFY `pitfall_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=16;

--
-- AUTO_INCREMENT for table `hair_symptoms`
--
ALTER TABLE `hair_symptoms`
...
--
ALTER TABLE `hair_types`
  MODIFY `hair_type_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=88;

--
-- AUTO_INCREMENT for table `method_age_suitability`
--
ALTER TABLE `method_age_suitability`
  MODIFY `suitability_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT for table `method_hair_type_compatibility`
--
ALTER TABLE `method_hair_type_compatibility`
  MODIFY `compatibility_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=79;

--
-- AUTO_INCREMENT for table `parent_child_activity_log`
--
ALTER TABLE `parent_child_activity_log`
  MODIFY `log_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=30;

--
-- AUTO_INCREMENT for table `pitfall_hair_type_applicability`
--
ALTER TABLE `pitfall_hair_type_applicability`
  MODIFY `applicability_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;
...
--
ALTER TABLE `product_age_appropriateness`
  MODIFY `appropriateness_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=143;

--
-- AUTO_INCREMENT for table `product_hair_type_compatibility`
--
ALTER TABLE `product_hair_type_compatibility`
  MODIFY `compatibility_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=63;
...
--
ALTER TABLE `routine_steps`
  MODIFY `step_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=166;

--
-- AUTO_INCREMENT for table `style_hair_type_compatibility`
--
ALTER TABLE `style_hair_type_compatibility`
  MODIFY `compatibility_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=58;

--
-- AUTO_INCREMENT for table `symptom_cause_mapping`
--
ALTER TABLE `symptom_cause_mapping`
  MODIFY `mapping_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=47;

--
-- AUTO_INCREMENT for table `treatment_solutions`
--
ALTER TABLE `treatment_solutions`
  MODIFY `solution_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;
...
--
ALTER TABLE `user_hair_concerns`
  MODIFY `user_concern_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=177;
...
--
ALTER TABLE `user_recommendations`
  MODIFY `recommendation_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=157;
...
--
-- Constraints for table `growth_milestones`
--
ALTER TABLE `growth_milestones`
  ADD CONSTRAINT `growth_milestones_ibfk_1` FOREIGN KEY (`profile_id`) REFERENCES `user_hair_profiles` (`profile_id`) ON DELETE CASCADE;

--
-- Constraints for table `hair_care_routines`
--
ALTER TABLE `hair_care_routines`
  ADD CONSTRAINT `hair_care_routines_ibfk_1` FOREIGN KEY (`profile_id`) REFERENCES `user_hair_profiles` (`profile_id`) ON DELETE CASCADE;

--
-- Constraints for table `hair_growth_progress`
--
ALTER TABLE `hair_growth_progress`
  ADD CONSTRAINT `hair_growth_progress_ibfk_1` FOREIGN KEY (`profile_id`) REFERENCES `user_hair_profiles` (`profile_id`) ON DELETE CASCADE;

--
-- Constraints for table `method_age_suitability`
--
ALTER TABLE `method_age_suitability`
  ADD CONSTRAINT `method_age_suitability_ibfk_1` FOREIGN KEY (`method_id`) REFERENCES `growth_methods` (`method_id`) ON DELETE CASCADE;

--
-- Constraints for table `method_hair_type_compatibility`
--
ALTER TABLE `method_hair_type_compatibility`
  ADD CONSTRAINT `method_hair_type_compatibility_ibfk_1` FOREIGN KEY (`method_id`) REFERENCES `growth_methods` (`method_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `method_hair_type_compatibility_ibfk_2` FOREIGN KEY (`hair_type_id`) REFERENCES `hair_types` (`hair_type_id`) ON DELETE CASCADE;

--
-- Constraints for table `parent_child_activity_log`
--
ALTER TABLE `parent_child_activity_log`
  ADD CONSTRAINT `parent_child_activity_log_ibfk_1` FOREIGN KEY (`parent_user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `parent_child_activity_log_ibfk_2` FOREIGN KEY (`child_user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE;

--
-- Constraints for table `pitfall_hair_type_applicability`
--
ALTER TABLE `pitfall_hair_type_applicability`
  ADD CONSTRAINT `pitfall_hair_type_applicability_ibfk_1` FOREIGN KEY (`pitfall_id`) REFERENCES `hair_pitfalls` (`pitfall_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `pitfall_hair_type_applicability_ibfk_2` FOREIGN KEY (`hair_type_id`) REFERENCES `hair_types` (`hair_type_id`) ON DELETE CASCADE;
...
--
ALTER TABLE `routine_completion_log`
  ADD CONSTRAINT `routine_completion_log_ibfk_1` FOREIGN KEY (`routine_id`) REFERENCES `hair_care_routines` (`routine_id`) ON DELETE CASCADE,
...
--
ALTER TABLE `routine_steps`
  ADD CONSTRAINT `routine_steps_ibfk_1` FOREIGN KEY (`routine_id`) REFERENCES `hair_care_routines` (`routine_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `routine_steps_ibfk_2` FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`) ON DELETE SET NULL,
  ADD CONSTRAINT `routine_steps_ibfk_3` FOREIGN KEY (`method_id`) REFERENCES `growth_methods` (`method_id`) ON DELETE SET NULL;
...
--
-- Constraints for table `symptom_cause_mapping`
--
ALTER TABLE `symptom_cause_mapping`
  ADD CONSTRAINT `symptom_cause_mapping_ibfk_1` FOREIGN KEY (`symptom_id`) REFERENCES `hair_symptoms` (`symptom_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `symptom_cause_mapping_ibfk_2` FOREIGN KEY (`cause_id`) REFERENCES `diagnosis_causes` (`cause_id`) ON DELETE CASCADE;

--
-- Constraints for table `treatment_solutions`
--
ALTER TABLE `treatment_solutions`
  ADD CONSTRAINT `treatment_solutions_ibfk_1` FOREIGN KEY (`cause_id`) REFERENCES `diagnosis_causes` (`cause_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `treatment_solutions_ibfk_2` FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`) ON DELETE SET NULL,
  ADD CONSTRAINT `treatment_solutions_ibfk_3` FOREIGN KEY (`method_id`) REFERENCES `growth_methods` (`method_id`) ON DELETE SET NULL;
...
ALTER TABLE `user_hair_concerns`
  ...
  ADD CONSTRAINT `user_hair_concerns_ibfk_2` FOREIGN KEY (`concern_id`) REFERENCES `hair_concerns` (`concern_id`) ON DELETE CASCADE;
...
ALTER TABLE `user_hair_profiles`
  ...
  ADD CONSTRAINT `user_hair_profiles_ibfk_2` FOREIGN KEY (`hair_type_id`) REFERENCES `hair_types` (`hair_type_id`) ON DELETE SET NULL;
...
ALTER TABLE `user_recommendations`
  ...
  ADD CONSTRAINT `user_recommendations_ibfk_3` FOREIGN KEY (`method_id`) REFERENCES `growth_methods` (`method_id`) ON DELETE SET NULL,
  ADD CONSTRAINT `user_recommendations_ibfk_4` FOREIGN KEY (`pitfall_id`) REFERENCES `hair_pitfalls` (`pitfall_id`) ON DELETE SET NULL,
